...
public class Assignment2 {
	...
	public String getStudentInfo(int sid) {
		try {
			String sqlQuery = "SELECT s.sid, s.slastname, s.sfirstname, s.sex, s.age, d.dname, s.yearofstudy, AVG(grade) AS average "
					+ "FROM A2.student s JOIN A2.studentCourse sc ON s.sid = sc.sid "
					+ "JOIN A2.department d ON d.decode = sc.dcode"
					+ "WHERE s.sid = ? GROUP BY s.sid, s.slastname, s.sfirstname, s.sex, s.age, d.dname, s.yearofstudy";
			PreparedStatement ps = connection.prepareStatement(sqlQuery);
			ps.setInt(1, sid);
			...
			String output = "";
			if (resultSet.next()) { // can this output have multiple rows?
	...
	public boolean switchDepartment(int sid, String oldDcode, String newDcode) {
		try {
			String sqlQuery = "SELECT dcode FROM A2.student WHERE sid = ?";
			PreparedStatement ps = connection.prepareStatement(sqlQuery);
			ps.setInt(1, sid);
		...
		return false; // do i need this here?
	}
	...
	public boolean updateDB() {
		try {
			...
			return true; // cause rows affected could be 0 if no male cs student in year 2
