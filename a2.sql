...
INSERT INTO query6 (fname, lname, cname, year, semseter)
(SELECT st.sfirstname as fname, st.slastname as lname, c.cname, cs.year, cs.semester 
FROM student st 
...
CREATE VIEW enrollment3CSAvg AS
	...
	JOIN studentCourse sc
		ON cs.csid = sc.csid
	WHERE dcode in csDcode  AND (cs.year, cs.semester) NOT IN (SELECT * FROM CurrentSem)
...
INSERT INTO query7(cname, semseter, year, avgmark)
(SELECT *
